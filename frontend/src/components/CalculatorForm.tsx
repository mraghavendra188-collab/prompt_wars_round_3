...
export function CalculatorForm({ onSubmit, loading }: Props) {
  ...
  const [fuelChanged, setFuelChanged] = useState(false);
  const [dietChanged, setDietChanged] = useState(false);
  ...
  return (
    <form className="card" onSubmit={handleSubmit} aria-labelledby="calc-heading">
      ...
      <fieldset>
        ...
        />
        <div className="field">
          <label htmlFor="car_fuel" className="visually-hidden">
            Car fuel type
          </label>
          <div className="input-wrapper">
            <select
              ...
              onChange={(e) => {
                setFuelChanged(true);
                patchTransport({ car_fuel: e.target.value as CarFuel });
              }}
            >
              {!fuelChanged && <option value="petrol">Car fuel type</option>}
              {FUEL_OPTIONS.map((o) => (
        ...
        <div className="field">
          ...
          <div className="input-wrapper">
            <select
              ...
              onChange={(e) => {
                setDietChanged(true);
                setInput((p) => ({ ...p, diet: e.target.value as DietType }));
              }}
            >
              {!dietChanged && <option value="medium_meat">Diet</option>}
              {DIET_OPTIONS.map((o) => (
                <option key={o.value} value={o.value}>
                  {o.label}
                </option>
